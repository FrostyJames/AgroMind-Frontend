import React, { useEffect, useState } from "react";
import { getAlerts } from "../services/alertService";

export default function ClimateAlerts() {
  const [alerts, setAlerts] = useState([]);
  const fallbackTips = [
    {
      title: "Sustainable Tip: Crop Rotation",
      description:
        "Rotate your crops annually to maintain soil fertility and reduce pest issues naturally.",
    },
    {
      title: "Water Management Reminder",
      description:
        "Clear drainage systems and use mulch to retain moisture during dry spells.",
    },
    {
      title: "Frost Protection Tip",
      description:
        "Cover sensitive crops with netting or move seedlings indoors overnight.",
    },
  ];

  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        const data = await getAlerts();
        setAlerts(data);
      } catch (error) {
        console.error("Failed to load alerts:", error);
      }
    };
    fetchAlerts();
  }, []);

  const displayAlerts = alerts.length > 0 ? alerts : fallbackTips;

  return (
    <section className="mb-8">
      <h3 className="text-lg font-bold text-green-700 mb-4">Climate Alerts & Tips</h3>
      <div className="space-y-4">
        {displayAlerts.map((alert, index) => (
          <div
            key={index}
            className="bg-red-50 border-l-4 border-red-400 p-4 rounded-md shadow-sm"
          >
            <p className="font-semibold text-red-700">
              {alert.event || alert.title}
            </p>
            <p className="text-sm text-gray-700 mt-1">
              {alert.description}
            </p>
          </div>
        ))}
      </div>
    </section>
  );
}